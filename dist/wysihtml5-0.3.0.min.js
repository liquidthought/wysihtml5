...
throw Error("getIframeWindow: No Document object found for iframe element");},getBody:function(a){return u.isHostObject(a,"body")?a.body:a.getElementsByTagName("body")[0]},getRootContainer:function(a){for(var b;b=a.parentNode;)a=b;return a},comparePoints:function(b,c,g,f){var j;if(b==g)return c===f?0:c<f?-1:1;if(j=e(g,b,!0))return c<=a(j)?-1:1;if(j=e(b,g,!0))return a(j)<f?-1:1;c=d(b,g);b=b===c?c:e(b,c,!0);g=g===c?c:e(g,c,!0);if(b===g)throw Error("comparePoints got to case 4 and childA and childB are the same!");
...
d=new y(a.focusNode,a.focusOffset),e="function"==typeof a.getName?a.getName():"Selection";if("undefined"!=typeof a.rangeCount)for(var g=0,j=a.rangeCount;g<j;++g)b[g]=v.inspect(a.getRangeAt(g));return"["+e+"(Ranges: "+b.join(", ")+")(anchor: "+c.inspect()+", focus: "+d.inspect()+"]"}b.requireModules(["DomUtil","DomRange","WrappedRange"]);b.config.checkSelectionRanges=!0;var p=b.dom,r=b.util,v=b.DomRange,o=b.WrappedRange,A=b.DOMException,y=p.DomPosition,z,C,x=b.util.isHostMethod(window,"getSelection"),
...
c){var d=v.getRangeDocument(c),d=b.createRange(d);d.collapseToPoint(c.endContainer,c.endOffset);a.nativeSelection.addRange(h(d));a.nativeSelection.extend(c.startContainer,c.startOffset);a.refresh()};s.addRange=Q?function(a,c){if(E&&w&&this.docSelection.type=="Control")k(this,a);else if(c&&M)J(this,a);else{var d;if(N)d=this.rangeCount;else{this.removeAllRanges();d=0}this.nativeSelection.addRange(h(a));this.rangeCount=this.nativeSelection.rangeCount;if(this.rangeCount==d+1){if(b.config.checkSelectionRanges)(d=
O(this.nativeSelection,this.rangeCount-1))&&!v.rangesEqual(d,a)&&(a=new o(d));this._ranges[this.rangeCount-1]=a;e(this,a,K(this.nativeSelection));this.isCollapsed=C(this)}else this.refresh()}}:function(a,b){if(b&&M)J(this,a);else{this.nativeSelection.addRange(h(a));this.refresh()}};s.setRanges=function(a){if(E&&a.length>1)n(this,a);else{this.removeAllRanges();for(var b=0,c=a.length;b<c;++b)this.addRange(a[b])}}}else if(r.isHostMethod(x,"empty")&&r.isHostMethod(P,"select")&&E&&F)s.removeAllRanges=
...
b="string"===typeof b?{rename_tag:b}:b}else if(a.firstChild)b={rename_tag:d};else return null;g=a.ownerDocument.createElement(b.rename_tag||g);var f={},j=b.set_class,k=b.add_class,i=b.set_attributes,m=b.check_attributes,n=h.classes,p=0,q=[];b=[];var r=[],v=[],t;i&&(f=wysihtml5.lang.object(i).clone());if(m)for(t in m)if(methodName=m[t],i="string"===typeof methodName?u[methodName]:methodName)i=i(c(a,t)),"string"===typeof i&&(f[t]=i);j&&q.push(j);if(k)for(t in k)if(i=o[k[t]])j=i(c(a,t)),"string"===typeof j&&
q.push(j);n["_wysihtml5-temp-placeholder"]=1;(v=a.getAttribute("class"))&&(q=q.concat(v.split(e)));for(k=q.length;p<k;p++)a=q[p],n[a]&&b.push(a);for(n=b.length;n--;)a=b[n],wysihtml5.lang.array(r).contains(a)||r.unshift(a);r.length&&(f["class"]=r.join(" "));for(t in f)try{g.setAttribute(t,f[t])}catch(l){}f.src&&("undefined"!==typeof f.width&&g.setAttribute("width",f.width),"undefined"!==typeof f.height&&g.setAttribute("height",f.height));return g},3:function(a){return a.ownerDocument.createTextNode(a.data)}},
...
"":a.replace(m,"")},numbers:function(a){return(a=(a||"").replace(n,""))||null}},q={left:"wysiwyg-float-left",right:"wysiwyg-float-right"},p={left:"wysiwyg-text-align-left",right:"wysiwyg-text-align-right",center:"wysiwyg-text-align-center",justify:"wysiwyg-text-align-justify"},r={left:"wysiwyg-clear-left",right:"wysiwyg-clear-right",both:"wysiwyg-clear-both",all:"wysiwyg-clear-both"},v={1:"wysiwyg-font-size-xx-small",2:"wysiwyg-font-size-small",3:"wysiwyg-font-size-medium",4:"wysiwyg-font-size-large",
5:"wysiwyg-font-size-x-large",6:"wysiwyg-font-size-xx-large",7:"wysiwyg-font-size-xx-large","-":"wysiwyg-font-size-smaller","+":"wysiwyg-font-size-larger"},o={align_img:function(a){return q[String(a).toLowerCase()]},align_text:function(a){return p[String(a).toLowerCase()]},clear_br:function(a){return r[String(a).toLowerCase()]},size_font:function(a){return v[String(a).charAt(0)]}};return function(a,c,d,e){wysihtml5.lang.object(h).merge(f).merge(c).get();for(var d=d||a.ownerDocument||document,c=d.createDocumentFragment(),
...
a.tagName.toLowerCase())&&d)return a;a=a.parentNode}return!1},postApply:function(a,b){for(var c=a[0],d=a[a.length-1],f=[],h,i=c,p=d,r=0,v=d.length,o,A,y=0,z=a.length;y<z;++y)if(o=a[y],A=this.getAdjacentMergeableTextNode(o.parentNode,!1)){if(h||(h=new e(A),f.push(h)),h.textNodes.push(o),o===c&&(i=h.firstTextNode,r=i.length),o===d)p=h.firstTextNode,v=h.getLength()}else h=null;if(c=this.getAdjacentMergeableTextNode(d.parentNode,!0))h||(h=new e(d),f.push(h)),h.textNodes.push(c);if(f.length){y=0;for(z=
...
c.copyStyles(f).from(k).to(this.disabledStylesHost);k.disabled=q;if(k.setActive)try{k.setActive()}catch(p){}else{var r=k.style,q=a.documentElement.scrollTop||a.body.scrollTop,v=a.documentElement.scrollLeft||a.body.scrollLeft,r={position:r.position,top:r.top,left:r.left,WebkitUserSelect:r.WebkitUserSelect};c.setStyles({position:"absolute",top:"-99999px",left:"-99999px",WebkitUserSelect:"none"}).on(k);k.focus();c.setStyles(r).on(k);d.scrollTo&&d.scrollTo(v,q)}c.copyStyles(h).from(k).to(this.focusStylesHost);
c.copyStyles(f).from(k).to(this.focusStylesHost);k.style.display=u;c.copyStyles(["display"]).from(k).to(this.iframe);var o=b.lang.array(h).without(["display"]);j?j.focus():k.blur();m&&k.setAttribute("placeholder",n);this.parent.observe("focus:composer",function(){c.copyStyles(o).from(g.focusStylesHost).to(g.iframe);c.copyStyles(f).from(g.focusStylesHost).to(g.element)});this.parent.observe("blur:composer",function(){c.copyStyles(o).from(g.blurStylesHost).to(g.iframe);c.copyStyles(f).from(g.blurStylesHost).to(g.element)});
this.parent.observe("disable:composer",function(){c.copyStyles(o).from(g.disabledStylesHost).to(g.iframe);c.copyStyles(f).from(g.disabledStylesHost).to(g.element)});this.parent.observe("enable:composer",function(){c.copyStyles(o).from(g.blurStylesHost).to(g.iframe);c.copyStyles(f).from(g.blurStylesHost).to(g.element)});return this}})(wysihtml5);
...
this.commandMapping,d=this.actionMapping,e,f,h;for(e in a)if(h=a[e],this.commandsDisabled?(f=!1,c.removeClass(h.link,"wysihtml5-command-active"),h.group&&c.removeClass(h.group,"wysihtml5-command-active"),h.dialog&&h.dialog.hide()):(f=this.composer.commands.state(h.name,h.value),b.lang.object(f).isArray()&&(f=1===f.length?f[0]:!0),c.removeClass(h.link,"wysihtml5-command-disabled"),h.group&&c.removeClass(h.group,"wysihtml5-command-disabled")),h.state!==f)(h.state=f)?(c.addClass(h.link,"wysihtml5-command-active"),
